...
fn render_node(node: &Node, defs: &LabelMap, counters: &Counters, prefix: &[u32], counter: &mut u32, out: &mut String) {
    match node {
        Node::Paragraph(inlines) => {
            if let [Inline::MathDisplay(m)] = inlines.as_slice() {
                match defs.get("mlmath") {
                    Some(def) => {
                        out.push_str(&substitute(&def.html_template, &[], &escape(m), None, None, &[]));
                        out.push('\n');
                    }
                    None => out.push_str(&format!("<div class=\"math-display\">\\[{}\\]</div>\n", escape(m))),
                }
            } else {
                out.push_str("<p>");
        ...
        Node::Block { label, args, id, .. } if label == "toc" => {
            // A ToC's content isn't its own (normally empty) children — it's
            // a nested listing generated from the whole document's
            // section/subsection headings, computed once up front in
            // `counters.toc` alongside everything else's numbers.
            if let Some(def) = defs.get("toc") {
                let toc_html = render_toc(&counters.toc);
                out.push_str(&substitute(&def.html_template, args, &toc_html, id.as_deref(), None, &[]));
                out.push('\n');
            }
        }
        Node::Block { label, args, id, children } => {
            ...
            match defs.get(label) {
                ...
                None => {
                    // no def file for this label: fall back to a same-named div
                    let id_attr = id.as_ref().map(|i| format!(" id=\"{i}\"")).unwrap_or_default();
                    out.push_str(&format!("<div class=\"{label}\"{id_attr}>\n{body}\n</div>\n"));
                }
            }
        }
    }
